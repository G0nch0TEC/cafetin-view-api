...
//    · avatar avatar-sm reemplaza .mini-avatar inline
// ══════════════════════════════════════════════════════
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Setear hoy
    const hoy    = new Date();
    selector.value = timestampAFechaInput(hoy.getTime());
    ...
    document.getElementById('btn-siguiente').addEventListener('click', () => {
        ...
        const hoyStr  = timestampAFechaInput(hoy.getTime());
        if (nueva <= hoyStr) {
...
// Avanza o retrocede N días desde un string YYYY-MM-DD
function offsetFecha(fechaStr, dias) {
    const d = new Date(fechaStr + 'T00:00:00');
    d.setDate(d.getDate() + dias);
    return timestampAFechaInput(d.getTime());
}
...
function actualizarBtnSiguiente(fechaStr) {
    ...
    if (!btn) return;
    btn.disabled = fechaStr >= timestampAFechaInput(Date.now());
}


// ── Aplicar filtros ───────────────────────────────────
function aplicarFiltros() {
    let resultado = _movimientosDia.slice();
    ...
    tbody.innerHTML = movimientos.map(m => `
        <tr>
            <td class="persona-cell">
                <span class="avatar avatar-sm">${iniciales(m.persona)}</span>
                ${m.persona}
            </td>
    ...
    if (tfoot) {
        ...
        if (subtotalFiado > 0) {
            filas.push(`<tr class="tfoot-row">
                ...
                <td colspan="2" class="tfoot-tipo"><span class="badge badge--fiado">FIADO</span></td>
            </tr>`);
        }
        if (subtotalPago > 0) {
            filas.push(`<tr class="tfoot-row">
                ...
                <td colspan="2" class="tfoot-tipo"><span class="badge badge--pago">PAGO</span></td>
            </tr>`);
        }
...
function exportarCSV() {
    if (!_movimientosDia.length) return;

    const fechaStr = document.getElementById('fecha-selector').value;
    const filas    = [['Persona', 'Tipo', 'Monto (S/)', 'Hora', 'Nota']];

    _movimientosDia.forEach(m => {
        filas.push([
            `"${m.persona}"`,
            m.tipo,
